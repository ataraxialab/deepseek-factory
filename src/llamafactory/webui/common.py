...
DEFAULT_CACHE_DIR = os.path.abspath("cache")
DEFAULT_CONFIG_DIR = os.path.abspath("config")
DEFAULT_DATA_DIR = os.path.abspath("data")
DEFAULT_SAVE_DIR = os.path.abspath("saves")
...
def get_init_config_path() -> os.PathLike:
    r"""
    Gets the path to user config.
    """
    return os.path.join("./", INIT_CONFIG)

def get_init_config(cfg: Dict, path: str, default):
    keys = path.split('.')
    current = cfg 
...
def gen_cmd(args: Dict[str, Any]) -> str:
    ...
    """
    cmd_lines = ["deepseekfactory-cli train "]
