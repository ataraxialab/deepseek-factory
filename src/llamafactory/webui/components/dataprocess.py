...
def create_dataprocess_tab(engine: "Engine") -> Dict[str, "Component"]:
    ...
    with gr.Row():
        with gr.Column(scale=9, elem_classes=["dropdown-button-container"]):
            dataset_src_path = create_dir_selector(base_path=initArgs.get("data_mount_dir", WORKSPACE), label="Dataset")
    ...
    with gr.Column() as segment_row:
        with gr.Row():
            with gr.Column(scale=9, elem_classes=["dropdown-button-container"]):
                train_name = create_dir_selector(base_path=initArgs.get("data_mount_dir", WORKSPACE), label="Traindir")
            with gr.Column(scale=9, elem_classes=["dropdown-button-container"]):
                test_name = create_dir_selector(base_path=initArgs.get("data_mount_dir", WORKSPACE), label="TestDir")
        with gr.Row():
    ...
    with gr.Group(visible=False) as distill_row:
        with gr.Row():
            with gr.Column(scale=9, elem_classes=["dropdown-button-container"]):
                dataset_dst_path = create_dir_selector(base_path=initArgs.get("data_mount_dir", WORKSPACE), label="DistillDir", showDirs=True)
        with gr.Row():
