...
def create_train_tab(engine: "Engine") -> Dict[str, "Component"]:
    ...
    with gr.Row():
        with gr.Column(scale=9, elem_classes=["dropdown-button-container"]):
            dataset = create_path_selector(base_path=initArgs.get("data_mount_dir", "/openr1_data"))
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            with gr.Row():
                with gr.Column(scale=9, elem_classes=["dropdown-button-container"]):
                    output_dir = create_path_selector(base_path=get_save_dir(f"train_{get_time()}"), label="output_dir", showDirs=True)
